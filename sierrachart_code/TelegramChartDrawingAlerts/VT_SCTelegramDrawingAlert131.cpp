...
int send_photo(const SCString& URL, const std::string& ChatID, const std::string& FilePath, const std::string& caption)
{
	...
	if (curl) 
	{
		...
		// Attempt the request for a maximum of 6 times
			...
			if (file) 
			{
				...
				while (std::getline(file, line)) 
				{
					...
					if(line.find("Type: Study") != std::string::npos)
					{
						...
						// Due to an inconsistency found with
						// studies that have Use As Main Price
						// Graph enabled For now it is necessary to
						// perform a check that will ensure the
						// study name is retrieved safely 
						//
						// variable used to inform the program of the inconsistency 
