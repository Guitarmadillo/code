...
{
	// Pause thread for 1 second to allow the screenshot file to be properly
...
" with your Sierra Chart username and I will add your username to the list of authorized users for this DLL. When your username is added to this list the file will appear in your Sierra Chart Data Folder the next time you start Sierra Chart."
		...
		"<br> <br> When a Chart Drawing Alert is triggered, a new line is written to a file in the <strong>C:\\SierraChart\\Logs</strong> directory. The path to this Folder is relative to where you have Sierra Chart installed. It is also possible that this Logs Folder does not exist yet. <span style=\"background-color: yellow; text-decoration: underline;\">Follow the next steps to ensure this folder exists.</span> "
	   ...
	   "<br> <br> Perform the following steps after you have inputted the Telegram Chat ID in Step 2. If you still see an Error in the Message Log about not being able to find the Logs folder, perform the following steps: "
		...
		"<br> <br> It is necessary to keep <span style=\"background-color: yellow; font-weight: bold;\">no more than 1</span> instance of this study per open chartbook. If the user added the study more than once by accident, this will result in the user recieving duplicate alerts on Telegram. This study contains a feature which can detect if another instance of this study exists within the same chartbook. "

		"<br> <br> 1. It is necessary to be on the newest version of the study. "
		"<br> 2. Enable Study Input #9 which is named <u><i>Find Duplicate Studies in Current Chartbook</i></u>."
		"<br> 3. Open the Sierra Chart Message Log by going to Window > Message Log."
		"<br> 4. Create and trigger a Chart Drawing or Chart Alert and monitor the Message Log."
		"<br> 5. Look out for a message that looks like this: "
		"<br> <br> \"A duplicate of Telegram Chart Drawing Alerts study is detected in the same chartbook. This duplicate was found on chart number (here it tells you the chart number). Reduce the number of instances of this study to one per chartbook, or you will recieve double alerts.\" " 

		"<br> <br> 6. Then you can proceed to Window > Windows and Chartbooks, select the chart that has this chart number and remove the study from this chart." 
	...
	else
	{
		...
		"1. You need to remove or comment out lines 322-323."
		...
		if(num_files_memory != number_of_files)
		{
			...
			if (file) 
			{
				...
				while (std::getline(file, line)) 
				{
					...
					//
					// THIS LOGIC APPLIES TO THE FORMAT FOR CHART DRAWING ALERTS
					// A DIFFERENT FORMATTING IS USED FOR CHART AND STUDY ALERTS
					//
					// TODO: IMPLEMENT AN IF ELSE IF TO HANDLE THE SEPARATE LOGIC FOR STUDY ALERT
					//
					//
					//
					if(line.find("Type: Study") != std::string::npos)
					{
						// TODO: Write different logic for the monospace text which will be for the formula 
						...
						//potentially have something here to take care of monospace formatting
						...
						if(source_start_pos != std::string::npos) // safety check
						{
							...
							std::string_view source_string(line.c_str() + source_start_pos, source_end_pos
							- source_start_pos);
						...
						if(sc_alert_chartbook == current_chartbook)
						{
							...
							// Go through each chart on the chartbook starting with chart number 1
							for(int ChartNumber = 1; ChartNumber < highest_chart_num; ChartNumber++)
							{
								// Check if the study with this name is found 
								int is_study_found = sc.GetStudyIDByName(ChartNumber, sc.GraphName.GetChars(), 0);

								// debug 
								/* msg.Format("Current Chart Number: %d Study ID returned for each chart in the book: %d", */ 
								/* sc.ChartNumber, is_study_found); */
								/* sc.AddMessageToLog(msg, 1); */

								// If the study is found and if the chart it was found on is not the current chart
								// This 
								if(is_study_found != 0 && ChartNumber != sc.ChartNumber)
								{
									// This should only return true if the study exists on two or more chartbooks
									// print which charts where the study is found
									msg.Format("A duplicate of Telegram Chart Drawing Alerts study is detected in the same "
									"chartbook. This duplicate was found on chart number %d. Reduce the number of studies "
									"per chartbook to one or you will recieve double alerts.", ChartNumber);
									sc.AddMessageToLog(msg,1);
								}
							}
							...
							else
							{
								...
								// If the file exists delete it. This is to
								// avoid conflict with the very first alert
								// that gets triggered 
								//
								// Sometimes if the file already exists it ends
								// up sending the previous file instead of the
								// new one. 
								...
								if (std::filesystem::exists(FilePath) && std::filesystem::is_regular_file(FilePath))
								{
									...
									// debug 
									/* sc.AddMessageToLog("New Log File: Image File deleted successfully!",1); */
								} 
								/* else */ 
								/* { */
								/* 	// debug */ 
								/* 	sc.AddMessageToLog("New Log File: Image File not found or not a regular file.",1); */
								/* } */
								...
								// This code is deprecated and does not make sense to use in this case
								// 
					...
					if (file) 
					{
						...
						while (std::getline(file, line)) 
						{
							...
							if(CountLines > NumberOfLines)
							{
								...
								// CHECK FOR IF ITS A CHART ALERT OR CHART DRAWING ALERT 
								if(line.find("Type: Study") != std::string::npos)
								{
									// TODO: Write different logic for the monospace text which will be for the formula 
									// instead of the price. 
