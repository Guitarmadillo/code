...
{
	// Pause thread for 1 second to allow the screenshot file to be properly saved.  
	Sleep(1000);
...
SCSFExport scsf_TelegramDrawingAlert(SCStudyInterfaceRef sc)
{
	...
	if (sc.SetDefaults)
	{
		...
		"<br> <br> The chart drawing will handle the alert and it's condition, but when the alert occurs, this study will be able to see that a new alert occurred by reading the log file."
	...
	else
	{
		...
		sc.AddMessageToLog("If you see this message it means you compiled the study on your own and"
		" it will be necessary for you to use your own bot using the provided input settings.",1);
		return; 
	...
	{
		sc.AddMessageToLog("Error: Specified Folder Does not Exist!",1);
		return;
	}
				...
				while (std::getline(file, line)) 
				{
					...
					if(sc_alert_chartbook == current_chartbook)
					{
						...
						if(Input_SendChartImage.GetBoolean() == 0)
						{
							...
							SCString query = std::move(object.dump()).c_str(); 						
							...
							if(sc.MakeHTTPPOSTRequest(std::move(URL), std::move(query), std::move(headers), 1) )
							{
								// successful request 
						...
						else
						{
							...
							SCString SC_FilePathName = std::move(std::string(FilePath).c_str());
